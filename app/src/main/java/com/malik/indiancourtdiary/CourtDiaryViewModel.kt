...
class CourtDiaryViewModel(a: Application) : AndroidViewModel(a) {
    ...
    fun add(raw: String, done: (String?) -> Unit) {
        ...
        if (!Regex("^[A-Z]{4}[0-9]{12}$").matches(cnr)) {
            done("Enter a valid 16-character CNR")
            return
        }
        ...
        viewModelScope.launch {
            ...
            try {
                val remote = CourtApiProvider.api.getCase(cnr)
                dao.save(
                    CourtCase(
                        cnr = remote.cnr,
                        caseTitle = remote.caseTitle,
                        courtName = remote.courtName,
                        nextHearingDate = remote.nextHearingDate,
                        stage = remote.stage
                    )
                )
                done(null)
                    ...
                    when (e.code()) {
                        404 -> "Case not found. Check the CNR number."
                        429 -> "Too many requests. Please try again later."
                        503 -> "Court service is not configured yet."
                        else -> "Court service error. Please try again."
                    }
                )
            } catch (e: IOException) {
                done("No internet connection or server unavailable.")
            } catch (e: Exception) {
                done("Could not sync this case. Please try again.")
            } finally {
